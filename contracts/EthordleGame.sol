...
contract EthordleGame {
    ...
    function getWord(address _player, uint256 _index) public view returns (string memory) {
        require(players[_player].id != 0, 'Player address does not exist');

        string memory word = players[_player].words[_index];

        return word;
    }

    function isWordUnique(string memory _word) public view returns (bool) {
        return words[_word].id == 0;
    }
}
